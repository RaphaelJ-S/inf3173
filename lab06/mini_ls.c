...
int main(int argc, char * argv[]) {
        ...
        for (i = 1; i < argc; i++) {
                ...
                printf("| %-12ld | %-11s | %-19ld | %-11s | %-6s | %-11ld | %-21s |", statbuf.st_ino, droits, type, statbuf.st_nlink, pw -> pw_name, gr -> gr_name, statbuf.st_size, Date_modification);
                if (strcmp(type, "répertoire") == 0) {
                        ...
                        readlink(argv[i], fichier_source, statbuf.st_size + 1);
                        if (stat(argv[i], & sb) == -1) {
                                printf("\033[1;31m");
                                printf(" %s ", argv[i]);
                                printf("\033[0m");
